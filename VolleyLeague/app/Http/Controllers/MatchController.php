...
class MatchController extends Controller
{
     ...
     */
    public function index(Request $request = null, string $league_id)
    {
		...
		//if(isset($request)){
			...
			if(isset($request)){
				...
				$request->validate([
				 ...
				 //Week days list -> $match->number_week & $match->dates
				...
				//$matches = Matchy::where("league_id",$league_id)->where('week_number', '>=', 2)->where('week_number', '<=', 3)->orderby('week_number')->get();
				//$matches = null;
